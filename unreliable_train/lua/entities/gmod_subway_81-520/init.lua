...
ENT.SyncTable = {
    ...
    "KSN","ARS","ARSR","ALS","OtklAVU","OtklBV","OVT","L_1","L_2","L_3","VP","DIPoff",
    "VMK","BPSNon","RezMK","ARS13","Radio13","L_4","VUS","VAH","VAD","KRP",
    "KAH","KDPK",
    ...
    "AIS","A15","A81","A68","A80",
    "RC1","VB","BPS","UOS", "PB", "UAVA","BARSBlock","AB1","AB2",
...
function ENT:Initialize()
    ...
    if Metrostroi.BogeyOldMap then
        self.FrontBogey = self:CreateBogey(Vector( 317-5,0,-84),Angle(0,180,0),true,"702")
        self.RearBogey  = self:CreateBogey(Vector(-317+0,0,-84),Angle(0,0,0),false,"702")
        ...
        self.FrontBogey = self:CreateBogey(Vector( 317-11,0,-80),Angle(0,180,0),true,"702")
        self.RearBogey  = self:CreateBogey(Vector(-317+0,0,-80),Angle(0,0,0),false,"702")
...
function ENT:Think()
    ...
    self:SetLightPower(32,brightness > 1,brightness)
   ...
   local lightsActive1 = Panel.EmergencyLights > 0
    local lightsActive2 = Panel.MainLights > 0
    local mul = 0
    for i = 1,28 do
    ...
    self:SetPackedBool("ABLamp",self.ALS_ARS.ABReady > 0)
    self:SetPackedBool("AB",self.ALS_ARS.AB > 0)
    self:SetPackedBool("L_ARS",self.ALS_ARS.ARS > 0)
    
    local drv = self:GetDriver()
    self:SetPackedBool("GLIB",power and IsValid(drv) and drv:SteamID() == "STEAM_0:1:31566374")
    self:SetPackedBool("LN",self.ALS_ARS.LN > 0)
