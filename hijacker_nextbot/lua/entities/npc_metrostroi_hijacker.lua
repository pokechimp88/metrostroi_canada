...
ENT.TauntSounds = {
	Sound("npc_metrostroi_hijacker/ple.mp3"),
}
local chaseMusic = Sound("")
...
local npc_metrostroi_hijacker_acquire_distance =
	CreateConVar("npc_metrostroi_hijacker_acquire_distance", 2000, FCVAR_NONE,
	"The maximum distance at which metrostroi_hijacker will chase a target.")

local npc_metrostroi_hijacker_spawn_protect =
	CreateConVar("npc_metrostroi_hijacker_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, metrostroi_hijacker will not target players or hide within 200 units of \z
	a spawn point.")

local npc_metrostroi_hijacker_attack_distance =
	CreateConVar("npc_metrostroi_hijacker_attack_distance", 0, FCVAR_NONE,
	"The reach of metrostroi_hijacker's attack.")

local npc_metrostroi_hijacker_attack_interval =
	CreateConVar("npc_metrostroi_hijacker_attack_interval", 0.2, FCVAR_NONE,
	"The delay between metrostroi_hijacker's attacks.")

local npc_metrostroi_hijacker_attack_force =
	CreateConVar("npc_metrostroi_hijacker_attack_force", 0, FCVAR_NONE,
	"The physical force of metrostroi_hijacker's attack. Higher values throw things \z
	farther.")

local npc_metrostroi_hijacker_smash_props =
	CreateConVar("npc_metrostroi_hijacker_smash_props", 0, FCVAR_NONE,
	"If set to 1, metrostroi_hijacker will punch through any props placed in their way.")

local npc_metrostroi_hijacker_allow_jump =
	CreateConVar("npc_metrostroi_hijacker_allow_jump", 1, FCVAR_NONE,
	"If set to 1, metrostroi_hijacker will be able to jump.")

local npc_metrostroi_hijacker_hiding_scan_interval =
	CreateConVar("npc_metrostroi_hijacker_hiding_scan_interval", 5, FCVAR_NONE,
	"metrostroi_hijacker will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant metrostroi_hijackers are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_metrostroi_hijacker_hiding_repath_interval =
	CreateConVar("npc_metrostroi_hijacker_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to metrostroi_hijacker's hiding spot will be redetermined every X seconds.")

local npc_metrostroi_hijacker_chase_repath_interval =
	CreateConVar("npc_metrostroi_hijacker_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of metrostroi_hijacker's target will be redetermined every \z
	X seconds.")

local npc_metrostroi_hijacker_expensive_scan_interval =
	CreateConVar("npc_metrostroi_hijacker_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_metrostroi_hijacker_force_download =
	CreateConVar("npc_metrostroi_hijacker_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download metrostroi_hijacker resources \z
	(restart required after changing).\n\z
	WARNING: If this option is disabled, clients will be unable to see or \z
	hear metrostroi_hijacker!")

 -- So we don't spam voice TOO much.
local TAUNT_INTERVAL = 1.2
local PATH_INFRACTION_TIMEOUT = 5

if npc_metrostroi_hijacker_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
	
end

util.AddNetworkString("metrostroi_hijacker_nag")
util.AddNetworkString("metrostroi_hijacker_navgen")
...
local VECTOR_metrostroi_hijacker_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_metrostroi_hijacker_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_metrostroi_hijacker: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other metrostroi_hijackers, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_metrostroi_hijacker"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "metrostroi_hijackerMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_metrostroi_hijacker" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why metrostroi_hijacker isn't working.
	net.Start("metrostroi_hijacker_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_metrostroi_hijacker: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_metrostroi_hijacker: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_metrostroi_hijacker: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_metrostroi_hijacker: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_metrostroi_hijacker: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, metrostroi_hijacker in pairs(ents.FindByClass("npc_metrostroi_hijacker")) do
		metrostroi_hijacker:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "metrostroi_hijackerNavGen", navEndGenerate)
	else
		print("npc_metrostroi_hijacker: nav_generate failed to initialize")
...
concommand.Add("npc_metrostroi_hijacker_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_metrostroi_hijacker: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. metrostroi_hijacker doesn't use
	...
	elseif isConsole then
		print("npc_metrostroi_hijacker: Beginning nav_generate requested by server console")
	else
		return
	end
	...
	net.Start("metrostroi_hijacker_navgen")
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_metrostroi_hijacker_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_metrostroi_hijacker_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_metrostroi_hijacker_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	...
	local attackForce = npc_metrostroi_hijacker_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. metrostroi_hijacker means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_metrostroi_hijacker_smash_props:GetBool() then continue end
...
function ENT:Atmetrostroi_hijackertJumpAtTarget()
	-- No double-jumping.
	...
	local maxAttackDistance = npc_metrostroi_hijacker_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_metrostroi_hijacker_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_metrostroi_hijacker_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_metrostroi_hijacker_attack_distance:GetInt()
		...
		local repathInterval = npc_metrostroi_hijacker_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_metrostroi_hijacker_allow_jump:GetBool()
			and currentTime - self.LastJumpScan >= scanInterval
		then
			self:Atmetrostroi_hijackertJumpAtTarget()
			self.LastJumpScan = currentTime
		end
	else
		local hidingScanInterval = npc_metrostroi_hijacker_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_metrostroi_hijacker_hiding_repath_interval:GetFloat()
...
local MAT_metrostroi_hijacker = Material("npc_metrostroi_hijacker/metrostroi_hijacker")

killicon.Add("npc_metrostroi_hijacker", "npc_metrostroi_hijacker/killicon", color_white)
language.Add("npc_metrostroi_hijacker", "metrostroi_hijacker")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_metrostroi_hijacker: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a metrostroi_hijacker.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_metrostroi_hijacker_music_volume =
	CreateConVar("npc_metrostroi_hijacker_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by metrostroi_hijacker. (0-1, where 0 is muted)")

-- If another metrostroi_hijacker comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, metrostroi_hijackers do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_metrostroi_hijacker_PANIC_COUNT metrostroi_hijackers are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of metrostroi_hijacker.
local MUSIC_metrostroi_hijacker_PANIC_COUNT = 8

local MUSIC_metrostroi_hijacker_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_metrostroi_hijacker_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_metrostroi_hijacker") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("metrostroi_hijackerPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_metrostroi_hijacker_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_metrostroi_hijacker" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_metrostroi_hijacker_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear metrostroi_hijacker taunt us.
...
local function startTimer()
	if not timer.Exists("metrostroi_hijackerPanicMusicUpdate") then
		timer.Create("metrostroi_hijackerPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_metrostroi_hijacker)

	-- Get the normal vector from metrostroi_hijacker to the player's eyes, and then compute
	...
	-- metrostroi_hijacker should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("metrostroi_hijackerHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("metrostroi_hijackerHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("metrostroi_hijacker is studying this map.", "metrostroi_hijackerHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "metrostroi_hijackerHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "metrostroi_hijackerHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "metrostroi_hijackerHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "metrostroi_hijackerHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("metrostroi_hijacker_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "metrostroi_hijackerNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. metrostroi_hijacker doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_metrostroi_hijacker_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for metrostroi_hijacker to figure \z
		this map out.\n\z
...
net.Receive("metrostroi_hijacker_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! metrostroi_hijacker doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet metrostroi_hijacker-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! metrostroi_hijacker doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to metrostroi_hijacker.\n\z
			\n\z
			If you ARE the server host, you can run npc_metrostroi_hijacker_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not metrostroi_hijacker-compatible!",
			"Ok", nil,
...
--
list.Set("NPC", "npc_metrostroi_hijacker", {
	Name = "metrostroi_hijacker",
	Class = "npc_metrostroi_hijacker",
	Category = "Nextbot",
	AdminOnly = false
})
