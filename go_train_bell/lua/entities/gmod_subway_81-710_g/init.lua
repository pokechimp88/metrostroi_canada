...
ENT.SyncTable = {
    ...
    "DriverValveBLDisconnect","DriverValveTLDisconnect","EPK","EmergencyBrakeValve","UAVA",
    "GV"
}
...
function ENT:Initialize()
    ...
    self.Lights = {
        ...
        -- Cabin
        [23] = { "dynamiclight",        Vector(432,-10.0,20), Angle(0,0,0), Color(255,125,25), brightness = 0.0005, distance = 600},

        [5] =   { "light",           Vector(465+5,-32, 48), Angle(0,0,0), Color(255,50,50),     brightness = 0.2, scale = 1.5, texture = "sprites/light_glow02.vmt" },
        [6] =   { "light",           Vector(465+5, 32, 48), Angle(0,0,0), Color(255,50,50),     brightness = 0.2, scale = 1.5, texture = "sprites/light_glow02.vmt" },
...
function ENT:Think()
    ...
    self:SetPackedBool("GaugeLights",self.Panel.GaugeLights > 0.5)
    local redlights = self.Panel.RedLights>0
    self:SetPackedBool("RedLight",redlights)
    self:SetLightPower(5,redlights)
    self:SetLightPower(6,redlights)

    local power = self.Panel.V1 > 0.5
    ...
    self:SetPackedBool("LRK",self.Panel.Sequence > 0)
    self:SetPackedBool("KT",self.Panel.KT > 0)
    self:SetPackedBool("AVU",self.Panel.AVU > 0.5)
    self:SetPackedBool("RingEnabled",self.Panel.Ring > 0.5)
    ...
    self:SetPackedBool("AR04",self.Panel.AR04 > 0)
    self:SetPackedBool("AR0",self.Panel.AR0 > 0)
    self:SetPackedBool("AR40",self.Panel.AR40 > 0)
    self:SetPackedBool("AR60",self.Panel.AR60 > 0)
    self:SetPackedBool("AR70",self.Panel.AR70 > 0)
    self:SetPackedBool("AR80",self.Panel.AR80 > 0)
    self:SetPackedBool("KT",self.Panel.KT)
    self:SetPackedBool("KVD",self.Panel.KVD > 0)
