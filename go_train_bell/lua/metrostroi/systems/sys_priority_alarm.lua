...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("priority_alarm")
...
if CLIENT then
  ...
  function TRAIN_SYSTEM:ClientThink()
    ...
    local messages = {
        ...
        DOORS = "DOORS OPEN",
        BPSN = "HEP OFF",
        PARKING = "PARK BRAKE ON",
        MANUAL = "MANUAL BRK ON",
        BRAKES = "AIR BRAKE ON",
        UAVA = "AUTOSTOP ACTV",
        UAVAK = "AUTOSTOP CONT",
        EPK = "EPV ACTIVATED",
    ...
    local colorb = Color(0,0,0,1)
    local colorf = Color(0,0,0,255)
    local colorh = Color(0,0,0,75)
    function TRAIN_SYSTEM:IGLA()
        local Train = self.Train
        surface.SetDrawColor(100*0.4,104*0.4,75*0.4)
        ...
        if State == -3 then
            self:PrintText(0,0,"PRIORITY ALARM",colorf)
            self:PrintText(0,1,"SYSTEM DAMAGED",colorf)
        elseif State == 0 then
        ...
        elseif State == 1 then
            ...
            self:PrintText(0,0,"ALARM SYSTEM LOAD",colorf)
        ...
        elseif State == 2 then
            ...
            if State2 == 0 then
                ...
                local ShowTime = Train:GetNW2Bool("IGLA:ShowTime")
                    local count = Train:GetNW2Int("IGLA:WagNumber")
                    self:PrintText(0,0,"UNIT",colorf)
                        self:PrintText(5,0,Format("%2d",count),colorf)
                        self:PrintText(10,0,"ACTIVE",colorf)
            ...
            elseif State2 == 2 then
                self:PrintText(0,0,"ENTER CODE:",colorf)
    ...
    function TRAIN_SYSTEM:Think(dT)
        ...
        elseif self.State == 2 then
            ...
            if self.State2 == 0 then
                if (self.Triggers.IGLA2 and self.Triggers.IGLA3) or self.StartMode == 1 then
                    self.Password = ""
                    self.State2 = 2
                    self.StandbyTimer = CurTime()
		    self.StartMode = 0
