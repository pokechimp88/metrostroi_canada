...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("81_717_CEM_Electric")
TRAIN_SYSTEM.MVM = 1
...
--]]
    ...
    if isLVZ then
        ...
        Train:WriteTrainWire(7,S["UO"]*Train.DoorBuzzer.Value+Train.BZOS.VH2)
        Train.BZOS.Power = T[10]*Train.A27.Value
        Train.BZOS.Ring = T[7]
	Panel.DoorBuzzer = T[7]
    end
    --[[if isPA then
    ...
    Panel.Ring = ARS.Ring+Train.Ring.Value
    ...
    Train.Panel.Headlights2 = S["F7"]*Train.VUS.Value*Train.A47.Value
    DLTimer = CurTime()*2%2 < 1 and 0 or 1
    Train.Panel.DitchLFlash = S["F7"]*Train.A47.Value*(Train.VUS.Value*(1-Train.ARS13.Value)+(Train.VUS.Value+Train.L_4.Value)*Train.ARS13.Value*DLTimer)
    Train.Panel.DitchRFlash = S["F7"]*Train.A47.Value*(Train.VUS.Value*(1-Train.ARS13.Value)+(Train.VUS.Value+Train.L_4.Value)*Train.ARS13.Value*(1-DLTimer))
    if isKSD then
