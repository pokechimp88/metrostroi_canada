...
function ENT:UpdateWagonNumber()
    ...
    for i=0,5 do
        ...
        if IsValid(leftNum) then
            leftNum:SetPos(self:LocalToWorld(Vector(200+i*6.6-count*6.6/2,61,7)))
            leftNum:SetSkin(num)
        end
        if IsValid(rightNum) then
            rightNum:SetPos(self:LocalToWorld(Vector(-220-i*6.6+count*6.6/2,-61,7)))
