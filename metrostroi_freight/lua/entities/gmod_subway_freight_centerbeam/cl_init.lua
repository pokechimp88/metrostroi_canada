...
function ENT:UpdateWagonNumber()
    ...
    for i=0,5 do
        ...
        if IsValid(leftNum) then
            leftNum:SetPos(self:LocalToWorld(Vector(195+i*6.6-count*6.6/2,54.1,7)))
            leftNum:SetSkin(num)
        end
        if IsValid(rightNum) then
            rightNum:SetPos(self:LocalToWorld(Vector(-197-i*6.6+count*6.6/2,-54.1,7)))
