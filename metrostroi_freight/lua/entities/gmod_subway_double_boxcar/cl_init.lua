...
function ENT:UpdateWagonNumber()
    ...
    for i=0,5 do
        ...
        if IsValid(leftNum) then
            leftNum:SetPos(self:LocalToWorld(Vector(216.4+i*6.6-count*6.6/2,57.2,18)))
            leftNum:SetSkin(num)
        end
        if IsValid(rightNum) then
            rightNum:SetPos(self:LocalToWorld(Vector(-216.5-i*6.6+count*6.6/2,-57.2,18)))
