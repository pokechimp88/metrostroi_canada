...
function ENT:UpdateWagonNumber()
    ...
    for i=0,5 do
        ...
        if IsValid(leftNum) then
            leftNum:SetPos(self:LocalToWorld(Vector(169+i*6.6-count*6.6/2,61.2,16)))
            leftNum:SetSkin(num)
        end
        if IsValid(rightNum) then
            rightNum:SetPos(self:LocalToWorld(Vector(-169-i*6.6+count*6.6/2,-61.5,16)))
